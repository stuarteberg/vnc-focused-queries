...
full_rsg as (
    (
      select
        ...
        cast(res / 8 * eval.from_a.origin.x as int64) as xa,
        cast(res / 8 * eval.from_a.origin.y as int64) as ya,
        cast(res / 8 * eval.from_a.origin.z as int64) as za,
        cast(res / 8 * eval.from_b.origin.x as int64) as xb,
        cast(res / 8 * eval.from_b.origin.y as int64) as yb,
        cast(res / 8 * eval.from_b.origin.z as int64) as zb,
        cast(res / 8 * point.x as int64) as x_nearby,
        cast(res / 8 * point.y as int64) as y_nearby,
        cast(res / 8 * point.z as int64) as z_nearby,

        from best_body_scores
          inner join vnc_rc4.rsg32 rsg on (best_body_scores.res = 32 and sv_a = rsg.label_a and sv_b = rsg.label_b)
    )
    union all
    (
      select
        ...
        from best_body_scores
          inner join vnc_rc4.rsg16 rsg on (best_body_scores.res = 16 and sv_a = rsg.label_a and sv_b = rsg.label_b)
    )
    union all
    (
      select
        ...
        from best_body_scores
          inner join vnc_rc4.rsg8 rsg on (best_body_scores.res = 8 and sv_a = rsg.label_a and sv_b = rsg.label_b)
    )
),
info_table as (
  select
    best_body_scores.res as res,
    rank_a, rank_b, rank,
    sv_a, sv_b,
    body_a, body_b, score,
  ...
  from best_body_scores
    left outer join full_rsg on (best_body_scores.res = full_rsg.res and sv_a = full_rsg.label_a and sv_b = full_rsg.label_b)
